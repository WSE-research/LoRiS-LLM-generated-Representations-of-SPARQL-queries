...
RUN (export DRY_RUN=True; streamlit run loris--llm-based-explanations-for-sparql-queries.py &) && sleep 5 && curl http://localhost:${SERVER_PORT}/
...
ENTRYPOINT ["sh", "-c", "\
    ...
    && streamlit run loris--llm-based-explanations-for-sparql-queries.py --server.port=$SERVER_PORT --server.address=0.0.0.0 --server.maxUploadSize=$SERVER_MAXUPLOADSIZE \
"]
