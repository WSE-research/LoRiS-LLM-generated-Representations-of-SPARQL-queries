...
def test_get_sparql_query_examples_parses_qald(monkeypatch):
    qald = {"questions": [
        {"query": {"sparql": "SELECT ?x WHERE { ?x a ?y . }"}},
        {"query": {}},  # no sparql -> skipped
    ]}
    ...
    monkeypatch.setattr(util.requests, "get", lambda url: _Resp())
    examples = util.get_sparql_query_examples()
    assert isinstance(examples, list)
    assert any("SELECT" in e for e in examples)


def test_get_sparql_query_examples_falls_back_on_error(monkeypatch):
    def _boom(url):
        raise RuntimeError("network down")

    monkeypatch.setattr(util.requests, "get", _boom)
    examples = util.get_sparql_query_examples()
